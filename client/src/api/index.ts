...
const API = axios.create({ baseURL: process.env.REACT_APP_SERVER_BASEURL_DEVELOPMENT });
...
export const fetchPost = (id: any, isWotd: boolean = false) => API.get(`/posts/${id}/?isWotd=${isWotd ? "YES" : "NO"}`);
export const fetchPosts = (page: any) => API.get(`/posts?page=${page}`);
export const fetchPostsBySearch = (searchQuery: any) => API.get(`/posts/search?searchQuery=${searchQuery.search || 'none'}`);
